...
public class ScalePerColumnOpcodeTests
{
    /// <summary>
    /// Builds a raw <c>ScalePerColumn</c> opcode body matching
    /// <c>dng_opcode_ScalePerColumn::PutData</c>: big-endian
    ...
    {
        var area = new DngRect(0, 0, 2, 4); // 4 columns, colPitch=1 -> 4 scales
        var body = BuildBody(area, plane: 0, planes: 1, rowPitch: 1, colPitch: 1,
            scales: [0.5f, 1.0f, 1.5f, 2.0f]);

        var p = ScalePerColumnOpcode.Decode(body);
    ...
    public void Decode_throws_on_count_mismatch()
    {
        ...
        Assert.Throws<DngException>(() => ScalePerColumnOpcode.Decode(body));
    }
    ...
    public void Decode_throws_on_non_finite_scale()
    {
        ...
        var body = BuildBody(area, 0, 1, 1, 1, scales: [float.NaN]);
        Assert.Throws<DngException>(() => ScalePerColumnOpcode.Decode(body));
    }

    [Fact]
    public void Apply_multiplies_per_column_scale_and_clips_to_unit_range()
    {
        ...
        // col 0: 0.5 * -4.0 = -2.0 -> clip to -1.0
        Assert.Equal(-1.0f, ReadPixel(image, 0, 0, 0));
        Assert.Equal(-1.0f, ReadPixel(image, 1, 0, 0));
        // col 1: 0.5 * 2.0 = 1.0
        Assert.Equal(1.0f, ReadPixel(image, 0, 1, 0), 5);
        // col 2: 0.5 * 4.0 = 2.0 -> clip to 1.0
    ...
    {
        // width=4, colPitch=2 -> scales apply to columns 0 and 2 only.
        var image = MakeImage(width: 4, height: 1, planes: 1, fill: 0.5f);
        var area = new DngRect(0, 0, 1, 4);
        var body = BuildBody(area, 0, 1, 1, 2, scales: [2.0f, 0.5f]);
        var p = ScalePerColumnOpcode.Decode(body);

        ScalePerColumnOpcode.Apply(image, p);

        Assert.Equal(1.0f, ReadPixel(image, 0, 0, 0), 5);
        Assert.Equal(0.5f, ReadPixel(image, 0, 1, 0), 5); // untouched (skipped by pitch)
        Assert.Equal(0.25f, ReadPixel(image, 0, 2, 0), 5);
        Assert.Equal(0.5f, ReadPixel(image, 0, 3, 0), 5); // untouched (skipped by pitch)
    }
    ...
    public void Apply_throws_not_yet_implemented_for_non_float_images()
    {
        ...
        Assert.Throws<DngException>(() => ScalePerColumnOpcode.Apply(image, p));
    }
}
