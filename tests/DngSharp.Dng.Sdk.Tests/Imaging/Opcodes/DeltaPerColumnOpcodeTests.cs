...
public class DeltaPerColumnOpcodeTests
{
    /// <summary>
    /// Builds a raw <c>DeltaPerColumn</c> opcode body matching
    /// <c>dng_opcode_DeltaPerColumn::PutData</c>: big-endian
    ...
    {
        var area = new DngRect(0, 0, 2, 4); // 4 columns, colPitch=1 -> 4 deltas
        var body = BuildBody(area, plane: 0, planes: 1, rowPitch: 1, colPitch: 1,
            deltas: [0.1f, 0.2f, 0.3f, 0.4f]);

        var p = DeltaPerColumnOpcode.Decode(body);
    ...
    public void Decode_throws_on_count_mismatch()
    {
        ...
        // building a body that claims 2 deltas for a 4-wide area — decode
        // must reject this as a framing mismatch.
        Assert.Throws<DngException>(() => DeltaPerColumnOpcode.Decode(body));
    }
    ...
    public void Decode_throws_on_non_finite_delta()
    {
        ...
        var body = BuildBody(area, 0, 1, 1, 1, deltas: [float.NaN]);
        Assert.Throws<DngException>(() => DeltaPerColumnOpcode.Decode(body));
    }

    [Fact]
    public void Apply_adds_per_column_delta_and_clips_to_unit_range()
    {
        var image = MakeImage(width: 3, height: 2, planes: 1, fill: 0.5f);
        var area = new DngRect(0, 0, 2, 3);
        var body = BuildBody(area, 0, 1, 1, 1, deltas: [-2.0f, 0.1f, 2.0f]);
        var p = DeltaPerColumnOpcode.Decode(body);

        DeltaPerColumnOpcode.Apply(image, p);

        // col 0: 0.5 + (-2.0) = -1.5 -> clip to -1.0
        Assert.Equal(-1.0f, ReadPixel(image, 0, 0, 0));
        Assert.Equal(-1.0f, ReadPixel(image, 1, 0, 0));
        // col 1: 0.5 + 0.1 = 0.6
        Assert.Equal(0.6f, ReadPixel(image, 0, 1, 0), 5);
        // col 2: 0.5 + 2.0 = 2.5 -> clip to 1.0
        Assert.Equal(1.0f, ReadPixel(image, 0, 2, 0));
    }

    [Fact]
    public void Apply_respects_col_pitch_stride()
    {
        // width=4, colPitch=2 -> deltas apply to columns 0 and 2 only.
        var image = MakeImage(width: 4, height: 1, planes: 1, fill: 0f);
        var area = new DngRect(0, 0, 1, 4);
        var body = BuildBody(area, 0, 1, 1, 2, deltas: [0.1f, 0.2f]);
        var p = DeltaPerColumnOpcode.Decode(body);

        DeltaPerColumnOpcode.Apply(image, p);

        Assert.Equal(0.1f, ReadPixel(image, 0, 0, 0), 5);
        Assert.Equal(0f, ReadPixel(image, 0, 1, 0), 5); // untouched (skipped by pitch)
        Assert.Equal(0.2f, ReadPixel(image, 0, 2, 0), 5);
        Assert.Equal(0f, ReadPixel(image, 0, 3, 0), 5); // untouched (skipped by pitch)
    }
    ...
    public void Apply_empty_delta_table_is_noop()
    {
        ...
        // count derived from Area.W() (0 for an empty rect) is 0 too.
    ...
    public void Apply_throws_not_yet_implemented_for_non_float_images()
    {
        ...
        Assert.Throws<DngException>(() => DeltaPerColumnOpcode.Apply(image, p));
    }
}
