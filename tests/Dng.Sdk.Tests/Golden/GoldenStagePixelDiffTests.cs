...
public class GoldenStagePixelDiffTests
{
    ...
    private static double ReadRawSample(ReadOnlySpan<byte> bytes, int index, Dng.Sdk.Pixels.PixelType type, int sampleSize)
    {
        int off = index * sampleSize;
        return type switch
        {
            Dng.Sdk.Pixels.PixelType.UInt8 => bytes[off],
            Dng.Sdk.Pixels.PixelType.SInt8 => (sbyte)bytes[off],
            Dng.Sdk.Pixels.PixelType.UInt16 => BinaryPrimitives.ReadUInt16LittleEndian(bytes.Slice(off, 2)),
            Dng.Sdk.Pixels.PixelType.SInt16 => BinaryPrimitives.ReadInt16LittleEndian(bytes.Slice(off, 2)),
            Dng.Sdk.Pixels.PixelType.UInt32 => BinaryPrimitives.ReadUInt32LittleEndian(bytes.Slice(off, 4)),
            Dng.Sdk.Pixels.PixelType.SInt32 => BinaryPrimitives.ReadInt32LittleEndian(bytes.Slice(off, 4)),
            Dng.Sdk.Pixels.PixelType.Float32 => BinaryPrimitives.ReadSingleLittleEndian(bytes.Slice(off, 4)),
            Dng.Sdk.Pixels.PixelType.Float16 => (double)BinaryPrimitives.ReadHalfLittleEndian(bytes.Slice(off, 2)),
